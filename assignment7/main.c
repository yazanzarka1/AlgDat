                          ...
                          const char *poi_filename)
{
  ...
  while (fgets(poiLine, 256, poiFile))
  {
    fileExtractResult_t poiData = extract_tokens_from_line(poiLine, 3);
    if (poiData.numberOfReadData >= 2)
    {
      token_t *data = poiData.data;
      uint32_t node_id = (uint32_t)data[0].value.number;
      if (node_id < graph->numVertices)
      {
        graph->vertex[node_id].poi_flags = (uint8_t)data[1].value.number;
        if (poiData.numberOfReadData == 3 && data[2].is_string)
        {
          graph->vertex[node_id].poi_name = malloc(strlen(data[2].value.string) + 1);
          strcpy(graph->vertex[node_id].poi_name, data[2].value.string);
        }
      }
      free_tokens(data, poiData.numberOfReadData);
    }
  }
  fclose(poiFile);
...
nearest_poi_t *dijkstra_nearest_pois(const graph_t *graph, const vertex_t *start, uint8_t poi_type_flags, int n, int *nodes_visited)
{
  init_predecessors(graph, start);
  int count = 0;
  pq_t *pq = create_pq(graph->numVertices);
                       ...
                       int num_landmarks)
{
  ...
  {
    int64_t fwd = (int64_t)landmarks[0]->weights[i * graph->numVertices + end_index] - (int64_t)landmarks[0]->weights[i * graph->numVertices + current_index];
    int64_t bwd = (int64_t)landmarks[1]->weights[i * graph->numVertices + current_index] - (int64_t)landmarks[1]->weights[i * graph->numVertices + end_index];
    if (fwd > best)
      best = fwd;
    if (bwd > best)
      best = bwd;
...
int main(int argc, char *argv[])
{
  ...
  if (argc == 4 && strcmp(argv[1], "dijkstra") != 0 && strcmp(argv[1], "alt") != 0 && strcmp(argv[1], "nearest") != 0 && strcmp(argv[1], "compare") != 0)
  {
